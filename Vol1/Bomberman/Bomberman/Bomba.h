...
private:
	...
	int tiempoAntesExplotar;

	Estado estado;

private://datos de la explosion

	int indiceEX, indiceEY;
	int altoExplosion, anchoExplosion;
public:
	...
	void dibujarBomba(Graphics ^g, Bitmap ^bmpBomba,int xJugador,int  yJugador, int **matriz)
	{
		...
		if (tiempoAntesExplotar == 6) { estado = Estado::explosion; }
	...
	void settiempo(int tiempo1) { tiempoAntesExplotar = tiempo1; }
	int getx() { return x; }
	int gety() { return y; }
	...
	Rectangle getRentangulo() { return Rectangle(x, y, 40, 40); }
};