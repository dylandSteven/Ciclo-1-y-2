...
public:
	...
	void colisionAmbulanciaAdvSaludable()
	{
		for (int i = 0; i < objAmbulancias->getCantidad(); i++)
		{
			for (int j = 0; j < objSaludable->getCantidad(); j++)
			{
				CAmbulancia* amb = objAmbulancias->getAmbulancias(i);
				CAdvSaludable* salu = objSaludable->getADVSaludable(j);

				if (amb->Zoom().IntersectsWith(salu->Zoom()))
				{
					objSaludable->eliminarEnemigo(j--);
					break;
				}
	...
	void colisionPoliciaAdvSaludable()
	{
		for (int i = 0; i < objPolicias->getCantidad(); i++)
		{
			for (int j = 0; j < objSaludable->getCantidad(); j++)
			{
				CPolicia* poli = objPolicias->getCPolicia(i);
				CAdvSaludable* salu = objSaludable->getADVSaludable(j);

				if (poli->Zoom().IntersectsWith(salu->Zoom()))
				{
					objSaludable->eliminarEnemigo(j--);
					break;
				}
	...
	void colisionAmbulanciaAdvAsintomatico()

	{
		for (int m = 0; m < objAmbulancias->getCantidad(); m++)
		{
			for (int e = 0; e < objAsintomaticos->getCantidad(); e++)
			{
				CAmbulancia* amb = objAmbulancias->getAmbulancias(m);
				CAdvAsintomatico* asinto = objAsintomaticos->getADVASintomatico(e);

				if (amb->Zoom().IntersectsWith(asinto->Zoom()))
				{
					objAsintomaticos->EliminarEnemigo(e--);
					break;
				}
			}
		}
	...
	void colisionPoliciaAdvAsintomatico()
	{
		for (int i = 0; i < objPolicias->getCantidad(); i++)
		{
			for (int j = 0; j < objAsintomaticos->getCantidad(); j++)
			{
				CPolicia* poli = objPolicias->getCPolicia(i);
				CAdvAsintomatico* asinto = objAsintomaticos->getADVASintomatico(j);


				if (poli->Zoom().IntersectsWith(asinto->Zoom()))
				{
					objAsintomaticos->EliminarEnemigo(j--);
					break;
				}
